...
export const createBuyer = async (req, res) => {
  try {
    ...
    return sendErrorResponse(res, 400, "VALIDATION_ERROR", "ไม่สามารถเพิ่มผู้ซื้อได้");
  }
};
...
export const updateBuyer = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, phone } = req.body;
    const updated = await Buyer.findByIdAndUpdate(id, { name, phone }, { new: true });
...
export const deleteBuyer = async (req, res) => {
  try {
    ...
    if (!deleted) {
      return sendErrorResponse(res, 404, "NOT_FOUND", "ไม่พบผู้ซื้อที่ต้องการลบ");
    }
